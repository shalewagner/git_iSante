...
#remove change to specifying tables ($TABLES argument removed below) - this didn't account for restoring all tables, which must be done separately to take this approach
mysqldump --defaults-file=$conffile --add-drop-table --skip-extended-insert --skip-quick $database 2> /dev/null \
