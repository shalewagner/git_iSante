...
Ext.onReady(function() {
	...
	{	
		...
		if(suffix == "Dt")
		{
			...
			suffix1 = txtArr[i].name.substring(txtArr[i].name.length-7);
			if(suffix1 == "FdateDt" || suffix1 == "AdateDt")
			{
				(Ext.get(txtArr[i].id)).on('blur', function(){
					errMsg =  Ext.util.validateDateFieldReferrals(document.getElementById(this.id),document.getElementById(this.id + 'Title'),'');
				...
				errMsg = Ext.util.validateDateFieldReferrals(document.getElementById(txtArr[i].id),document.getElementById(txtArr[i].id + 'Title'),'');
