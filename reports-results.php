...
function findWeekRange($weekBack, &$startWeekDate){
  $weekBack = $weekBack . " Sunday";
  $weekCalc = "SELECT STR_TO_DATE(?, ?) AS weekStart";
  $fetchWeekRange = database()->query($weekCalc, array($weekBack, '%X%V %W'))->fetchAll(PDO::FETCH_ASSOC);
...
$sqlByTime = "select l.indicator, l.indicatorType, $findDenom name$lang, definition$lang," . $periodsInQuery . "
from dw_".$reportType."ReportLookup l LEFT JOIN dw_" . $reportType . "_slices s ON org_unit = ? and time_period = ? and s.indicator = l.indicator WHERE l.indicator > 0 " . $limitIndicators . " GROUP BY l.indicator";
$resultByTime = databaseSelect()->query($sqlByTime, array($orgUnit, $timePeriod))->fetchAll(PDO::FETCH_ASSOC);

// The subrow (aka detail) query
if ( $orgUnit != 'Haiti') {
  if ($orgUnit == 'SiteCode') {
   // Subrow query for clinics
    $sqlByTimeSubrow = "select s.indicator, c.clinic, c.siteCode, l.indicatorType, $findDenom name$lang," . $periodsInQuery . " from dw_" . $reportType . "_slices s, dw_" . $reportType . "ReportLookup l, clinicLookup c where org_unit = '$orgUnit' and time_period = ? and s.indicator = l.indicator and c.siteCode = s.org_value and s.indicator > 0 GROUP BY c.clinic, s.indicator";
  } else {
    // Subrow query for network, department, commune
    $sqlByTimeSubrow = "select s.indicator, org_value, l.indicatorType, $findDenom name$lang, " . $periodsInQuery . " from dw_" . $reportType . "_slices s, dw_" . $reportType . "ReportLookup l where org_unit = ? and time_period = ? and s.indicator = l.indicator and s.indicator > 0 GROUP BY s.org_value, s.indicator"; 
  }
