...
#    $main::isanteSource = '/Volumes/heart/Users/dad/git_iSante';
...
=begin comment
  ...
  The send side is automatically set up in the standard iSanté install
...
print "\nFiguring out replication targets\n";
...
#   modified in 15.1 to send to I-TECH consolidated and UGP consolidated by default; use the replicationTargets config item to modify this behavior
    $replicationTargets =
'itechConsolidated,https://isante-consolidated.cirg.washington.edu/receiver/receive-file.pl,identified,papConsolidated,https://isante.ugp.ht/consolidatedId/receiver/receive-file.pl,identified';
}
...
# unload database, using first target as basis for filenames  
my $currentTargetName = shift(@replicationTargetParts);
...
if ( $deidentifyMode eq 'identified' ) {
    ...
    die
"\nreplicate_database no longer supports deidentified unload -- \nunload manually using readSource.pl with the deidentify flag\n";
    $currentDeidentify       = 1;
...
my $errorFileName;
{
    ...
    my $dateTimeFile = sprintf(
        "%04d-%02d-%02d_%02d",
        $timeParts[5] + 1900, $timeParts[4] + 1,
        $timeParts[3],        $lastReadUniqueNumber
    );

    my $databaseName = 'itech';
    $dataFileName =
        "$dataFilePath/$dateTimeFile-$serverId" . '_'
      . "$databaseName-$currentTargetName"
      . '.csv.gz';
...
{
    my $reportFile;
    open( $reportFile, '<', $reportFileName )
...
#Using the unloaded information, post all targets in the database
@replicationTargetParts = split( /,/, $replicationTargets );
while ( my $currentTargetName = shift(@replicationTargetParts) ) {
    my $currentTargetUrl = shift(@replicationTargetParts);
    my $deidentifyMode   = shift(@replicationTargetParts);

    #make a record in replicationRead that documents this read 
    print "\nAdding replicatinRead record\n";
    {
        $dbHandle->do( '
        insert into replicationRead 
         ...
         values (?,?,?,?,?,?,?,?,?)', undef,
            $currentTargetName,       $currentTargetUrl,
            timeToSqlDateTime($lastReadTime),
            timeToSqlDateTime($mostRecentChangeTime), $dataFileName,
            $reportFileName,
...
#Attempt to transmit any files that have not been transmitted. 
#Mark them as transmitted on success and move them into $dataFileSentPath
{
    my $allTargets = $dbHandle->selectcol_arrayref( '
       select distinct targetName from replicationRead' );

#Handle each target separately. 
#If we are sending data to two different machines and one is not responding, the other will still get data.
    ...
    foreach my $targetName (@$allTargets) {
        ...
        my $untransmittedRecords = $dbHandle->selectall_arrayref( '
           ...
           ', { Slice => {} }, 0, $targetName )
          ;    #the Slice thing makes it an array of hashes

        foreach my $untransmitted (@$untransmittedRecords) {   
            print "\nAttempting to send file with curl\n";
            my $durationStart = time();
            my $command =
                'curl -k -3 --user '
              . join( ':', consolidatedUserPass() )
              . ' --url '
              . $untransmitted->{targetUrl}
              . ' --form "startTime='
              . $untransmitted->{startTime} . '"'
              . ' --form "stopTime='
              . $untransmitted->{stopTime} . '"'
              . ' --form "serverName='
              . $serverId . '"'
              . ' --form "databaseName=itech"'
              . ' --form "targetName='
              . $untransmitted->{targetName} . '"'
              . ' --form "dataFileName=@'
              . $untransmitted->{dataFileName} . '"'
              . ' --form "reportFileName=@'
        ...
        #if all replicationRead records are transmitted, OK to move file to sent 
        print "\nCopying file to sentpath\n";
            my $allSent = $dbHandle->selectcol_arrayref(
'select COUNT(*) as cnt from replicationRead where transmitted = 0'
            );
            foreach my $sent (@$allSent) {
                print '\n***' . $sent . '***\n';
                if ( 0+$sent == 0 ) {
                        print "\n" . $dataFileSentPath . " " . $untransmitted->{dataFileName} . "\n";
                    map { `mv "$_" "$dataFileSentPath"` } (
                        $untransmitted->{dataFileName},
                        $untransmitted->{reportFileName},
                        $untransmitted->{errorFileName}
                    );
