...
Ext.onReady(function() {
	...
	Ext.get('riskFactPres0').on('blur',function(){
		Ext.util.enableAssociated(rf_pres0, rfArr);
	});

	Ext.get('riskFactPres1').on('blur',function(){
		Ext.util.disableAssociated(rf_pres0, rfArr);
	});
	...
	Ext.get('famPlan1').on('click',function(){
       ...
       var prevObs = new Array('oldGestWeeks', 'gravida', 'aborta', 'para');
        for(var f = 0; f < prevObs.length;f++)
        {
                var tempText = new Ext.form.TextField({
                      fieldLabel: '',
                      maskRe : /[\d{1,2}\/]/,
                      validationEvent: false,
                      allowBlank:true
                });
		tempText.applyToMarkup(prevObs[f]);

                Ext.get(prevObs[f]).on('blur', function(){
                ...
                errMsg = Ext.util.isValueInBound(document.getElementById(prevObs[f]), document.getElementById(prevObs[f] + 'Title'), 1, '', '');
                errCount = Ext.util.showErrorHead(errFields, errMsgs, this.id, errMsg, errCount);
        }
...
/*	menopauseAge = new Ext.form.TextField({
	...
	Ext.get('menopauseAge').on('blur', function(){
		errMsg = Ext.util.validateAge(document.mainForm.menopauseAge,document.getElementById("menopauseTitle"),'');
...
*/	
	...
	Ext.get('motherPregWeeks').on('blur', function(){
		errMsg = Ext.util.isValueInBound(document.mainForm.motherPregWeeks,document.getElementById("motherPregWeeksTitle"),'0',45,'');
